...
export async function sendAdminAddedEmail(
  email: string,
  tenant: { name: string; slug: string; logo_url: string | null; primary_color: string }
): Promise<boolean> {
  const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
  const loginLink = `${frontendUrl}/login?tenant=${tenant.slug}`;
  
  const brandColor = tenant.primary_color || '#3b82f6';
  const textColor = getContrastColor(brandColor);
  const logoSrc = tenant.logo_url || 'https://i.imgur.com/KzWdOaH.png';
  
  const subject = `Designation: Administrator of ${tenant.name}`;
  ...
  <div class="wrapper">
    <div class="container">
      ...
      <div class="content">
        ...
        <p class="subtitle">
          You have been designated as an Administrator for <strong>${tenant.name}</strong> on the Matchup platform.
        </p>
        ...
        <div class="invite-box">
          ...
          <div class="info-row">
            ...
            <div class="info-value" style="font-weight: 600; color: ${brandColor};">Administrator</div>
          </div>
          <div class="info-row">
        ...
        <a href="${loginLink}" class="cta-btn">Access Admin Dashboard</a>
      </div>
      
      <div class="footer">
        This is an official designation notification on behalf of system administration.<br><br>
        Powered by <a href="${frontendUrl}">Matchup</a>
      </div>
    </div>
  </div>
